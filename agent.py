...
class Agent():
    ...
    def __init__(self, hyperparameter_set):
        ...
        self.episodes = hyperparameters.get('episodes', 10000)  # Number of episodes to run, default to 10000 if not specified
    ...
    def run(self, is_training=True, render=False, resume=False):
        if is_training:
            ...
            log_message = f"{start_time.strftime(DATE_FORMAT)}: {'Resuming' if resume else 'Starting new'} training..."
            print(log_message)
            with open(self.LOG_FILE, 'a' if resume else 'w') as file:
        ...
        # Load the previous model if resuming
        if resume and os.path.exists(self.MODEL_FILE):
            try:
                checkpoint = torch.load(self.MODEL_FILE)
                policy_dqn.load_state_dict(checkpoint['model_state_dict'])  # Note the change here
                print(f"Successfully loaded model from {self.MODEL_FILE}")
            except Exception as e:
                print(f"Error loading model: {e}")

        if is_training:
            # Initialize epsilon - possibly with a lower value if resuming
            epsilon = self.epsilon_init
            if resume:
                # You might want to start with a lower epsilon when resuming
                epsilon = max(self.epsilon_init * 0.5, self.epsilon_min)
            ...
            # Load learned policy
            checkpoint = torch.load(self.MODEL_FILE)
            policy_dqn.load_state_dict(checkpoint['model_state_dict'])  # Extract just the model state

            # switch model to evaluation mode
            policy_dqn.eval()

        # If resuming, load the saved state
        if resume and is_training:
            checkpoint = torch.load(self.MODEL_FILE)
            ...
            # Load replay memory if it was saved
            if checkpoint.get('replay_memory') is not None and hasattr(memory, 'load_state_dict'):
        ...
        try:
                ...
                if is_training:
                    if episode_reward > best_reward:
                        ...
                        checkpoint = {
                            ...
                            # Only save replay memory if it supports state_dict()
                            'replay_memory': memory.state_dict() if hasattr(memory, 'state_dict') else None
                        }
            ...
            if is_training:
                ...
                # Also save the graph with current progress
                self.save_graph(rewards_per_episode, epsilon_history)
                print(f"Training graph saved to {self.GRAPH_FILE}")
    ...
    def save_training_state(self, episode, best_reward, epsilon):
        state = {
            'episode': episode,
            'best_reward': best_reward,
            'epsilon': epsilon,
        ...
        state_file = os.path.join(RUNS_DIR, f'{self.hyperparameter_set}_state.pkl')
...
if __name__ == '__main__':
    ...
    parser.add_argument('hyperparameters', help='Hyperparameter set name from hyperparameters.yaml')
    parser.add_argument('--train', help='Training mode', action='store_true')
    parser.add_argument('--resume', help='Resume training from saved model', action='store_true')
